...
nonzero = sum(tmp_z!=0)
DF = (nonzero+1) + sum(diag(Hatmat))+2*ns
eval(parse(text = paste0('AIC', seed, ' = 2*like + 2*DF ')))
eval(parse(text = paste0('AICc',seed, ' = 2*like + 2*DF + 2*DF*(DF+1)/(ns*nt-DF-1)'))) 
eval(parse(text = paste0('BIC', seed, ' = 2*like  + log(nt*ns)*DF ')))


eval(parse(text = paste0("save.image(file =","'", paste0('./hybrid_simulation/result_lamt',lam_t,"_lams",lam_s,"_",seed, '.RData',"')"))))
