...
tvec0 = rep(0,p*nt+2*ns); length(tvec0)
tmp_loc = c()
for ( i in 1:ns)
{
  x = xlist[[i]]
  ...
  tvec0[p*nt+2*i-1] = fit$estimate[2]
  tvec0[p*nt+2*i] = fit$estimate[3]
}
tmp_beta = lm(tmp_loc~Z-1)$coefficients
tvec0[1:(p*nt)] = rep(tmp_beta,nt)
...
hygev_m = function(tvec, lam_s, xlist, zlist, Om, dmatrix, rho, z_init_list, u_init_list)  
{
  ...
  for ( i in 1: ns){
    ...
    loc.vec = c()
    for ( tt in 1:nt){
      beta.t = tvec[(1:p)+p*(tt-1)]
      loc.vec[tt] = z%*%beta.t
    }
    sc.vec = tvec[p*nt+2*i-1]
    sh.vec = tvec[p*nt+2*i]
...
for (iiter in 1:maxiiter){
  ...
  for (i in 1:ns) {
    ...
    AA = rho*t(A)%*%B
    sk[i] = base::norm(drop(AA%*%(tmp_z -z_init)),"2")   # dual
    rk[i] = base::norm(drop(A%*%old_mu + B%*%tmp_z),"2") # primal
  ...
  if (max(rk)<epri & max(sk) <edul) break
  
  
  if (iiter %% 100 == 0){
    mid_time = Sys.time()
    cat(lam_t,"*****iter:::", iiter, "/log:", mid_time-start_time  ,"\n")
  }
...
eval(parse(text = paste0("save.image(file =","'", paste0('./RData_hybrid3_real/hybrid3_',lam_t,"_",lam_s, '.RData',"')"))))
