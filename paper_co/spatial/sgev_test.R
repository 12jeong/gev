...
# load data
load("./Rexport/RData_setting1_2/numerical_setting.RDa")
file_count = length(lambdaset)
for (i in 1:file_count){
  eval(parse(text = paste0('load("./Rexport/RData_setting1_2/result_train', i, '.RDa")')))
}
...
# plotdf = data.frame(lambda=rep(lambdaset,2),vec=c(AICvec,BICvec), group=rep(c("AIC","BIC"),each=11))
plotdf = data.frame(lambda=lambdaset,AIC=AICvec,BIC=BICvec)
